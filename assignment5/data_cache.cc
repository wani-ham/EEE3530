...
void data_cache_t::read(inst_t *m_inst) {
    ...
    /* Assignment 5: Cache
       Modify the following code lines to search set-associative cache blocks. */
    block_t *block = &blocks[set_index][0];
    bool flag = false;  // true: cache hit, false: cache miss
    for(int i = 0; i < 4; i++) {  // check for all 4 ways
        block_t *tmp_block = &blocks[set_index][i];
        if(tmp_block->valid && (tmp_block->tag == tag)) { // if cache hit 
            flag = true; 
            block = tmp_block;  // saves tmp_block to block
            break;
        }
    }
    if(!flag) { block = 0; }  // if cache miss, save null in block
...
void data_cache_t::handle_response(int64_t *m_data) {
    ...
    /* Assignment 5: Cache
       Modify the following code line to find an LRU block. */
    int alloc_way = 0;  // allocator way number
    uint64_t min_la_time = ~(uint64_t)0;  // min last access, initialize to possible maximum value
    min_la_time <<= 1;
    for(int i = 0; i < 4; i++) {
        block_t *block = &blocks[set_index][i];
        uint64_t la_time = block->last_access;  // last access time of block
        if(la_time < min_la_time) {  // the smallest last access time is the oldest
            min_la_time = la_time;  // update minimum la time
            alloc_way = i;  // save current way
        }
    }
    block_t *allocator = &blocks[set_index][alloc_way];  // replace the cache block in alloc_way
